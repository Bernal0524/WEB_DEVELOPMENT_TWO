...
export default function Modal() { 
  ...
  // Recuperando funciones del slice de favoritos
  const handleClickFavorite = useAppStore((state) => state.handleClickFavorite);
  const favoriteExists = useAppStore((state) => state.favoriteExists);

  // Función para renderizar la lista de ingredientes de una bebida 
              ...
              > 
                <DialogPanel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:p-6"> 
                  ...
                  <div className='mt-5 flex justify-between gap-4'>  
                    ...
                    {/* Modificación del botón "Agregar a favoritos" */}
                    <button 
                      type='button' 
                      onClick={() => {
                        handleClickFavorite(selectedRecipe); // Llamar a la función handleClickFavorite
