export function WatchedMoviesContainer({ children }) {
  ...
  export function WatchedMoviesList({ watched, onDeleteMovie }) { // Agrega onDeleteMovie
    return (
      <ul className="list">
        {watched.map((movie) => (
          <WatchedMovie movie={movie} key={movie.imdbID} onDeleteMovie={onDeleteMovie} /> // Pasa onDeleteMovie
        ))}
      </ul>
    );
  }
  
  export function WatchedMovie({ movie, onDeleteMovie }) { // Recibe onDeleteMovie
    return (
      <li>
        ...
        <div>
          ...
          <button className="btn-delete" onClick={() => onDeleteMovie(movie.imdbID)}> {/* Llama a onDeleteMovie */}
            X
          </button>
        </div>
      </li>
    ...
    return (
      <div className="summary">
        ...
        <div>
          ...
          <p>
            <span>⏳</span>
            <span>{avgRuntime.toFixed(0)} min</span> {/* Redondea a entero */}
          </p>
        </div>
      </div>
    );
